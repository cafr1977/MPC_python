...
settings['pollutant'] = 'O3'  # make sure this matches the column name in the reference data file (CSV or XLSX)
settings['unit'] = 'ppb'  # concentration units of the target pollutant (for plot labels)
settings['time_interval'] = 60
...
settings['sensors_included'] = ['E2VO3','Temperature','Humidity']  # list the sensors you want in the model (both pollutant and environmental, like temperature or humidity)
settings['scaler_type'] = StandardScaler()  # How the data is scaled. StandardScaler is mean zero and st dev 1
...
settings['weighting_percentile'] = [75]  # list which percentiles to test for weighting. All data points in that percentile or higher will be weighted higher than those below.
...
settings['preprocess'] = ["rmv_warmup", 'temp_C_2_K', 'rmvJuly16-17']
...
settings[
    'quartiles_downsampling_rate'] = 0.5  ## If using 'resample_quartile', choose a downsampling rate between 0-1 (e.g., keeping 70% of instances within the lower quartile)
...
settings['column_names'] = {'3.1.0':
    ...
    ,
                            ...
                            'whittier':
                            ["PodID","date","time",'unk1','unk2','Temperature','Humidity','CO2','blank1','blank2','unk3',
                             'unk4','unk5','unk6','unk7','unk8','unk9','unk_10','sensor1','sensor2','sensor3','sensor4',
                             'sensor5','sensor6','sensor7','sensor8','blank3','extrablank'],
...
if ('rf_pieceweight' in settings['models'] or 'lin_reg_pieceweight' in settings['models'] or 'xgb_pieceweight' in
    settings['models']) and len(settings['models']) != 1:
...
if settings['colo_run_name'] == '':
    ...
    output_folder_name = f'Output_{settings["colo_run_name"]}'
...
colo_file_list = deployment_log[(deployment_log['deployment'] == 'C') & (deployment_log['pollutant'] == settings['pollutant'])][
    'file_name']  # list of all colo files to combine

# make sure there just one colocation pod
settings['colo_pod_names'] = list(set([string.split('_')[0] for string in colo_file_list]))
if len(settings['colo_pod_names']) == 0:
...
settings['scaler'] = {pod: None for pod in settings['colo_pod_names']}

for q, colo_pod_name in enumerate(settings['colo_pod_names']):
    # load pod data
    ...
    if colo_pod_data.empty:
        raise AssertionError(
            f"No colocation data was found in the Colocation Pod folder for pod {colo_pod_name} that matched the deployment log. Stopping execution.")
    ...
    print('Preprocessing colocation pod data...')
    colo_pod_data = preprocessing_func.preprocessing_func(colo_pod_data, settings['sensors_included'], settings['t_warmup'],
                                                          settings['preprocess'])
    ...
    if "binned_resample" in settings['preprocess']:
        X_train, y_train = preprocessing_func.binned_resample(X_train, y_train, settings['n_bins'],
                                                              settings['binned_resample_binnum_multiplier'])
    ...
    if "resample_quartile" in settings['preprocess']:
        for quartile in settings['quartiles_to_resample']:
            X_train, y_train = preprocessing_func.resample_quartile(X_train, y_train, quartile,
                                                                    settings['quartiles_downsampling_rate'])

    # Fit the scaler on training data (only selected columns)
    settings['scaler'][colo_pod_name] = settings['scaler_type']
    settings['scaler'][colo_pod_name].fit(X_train[settings['sensors_included']])

    # Transform and replace only selected columns
    X_train[settings['sensors_included']] = settings['scaler'][colo_pod_name].transform(X_train[settings['sensors_included']])
    X_test[settings['sensors_included']] = settings['scaler'][colo_pod_name].transform(X_test[settings['sensors_included']])

    # X_train = X_train.astype(np.float32)
    # X_test = X_test.astype(np.float32)

    X_std = X.copy()
    X_std[settings['sensors_included']] = settings['scaler'][colo_pod_name].transform(X[settings['sensors_included']])
    ...
    X_std.to_csv(os.path.join('Outputs', output_folder_name, f'colo_X_std_{colo_pod_name}.csv'))
    y_df.to_csv(os.path.join('Outputs', output_folder_name, 'colo_y_reference.csv'))
    X.to_csv(os.path.join('Outputs', output_folder_name, f'colo_X_{colo_pod_name}.csv'))
    ...
    if settings['models'] == ['rf_pieceweight'] or settings['models'] == ['lin_reg_pieceweight'] or settings['models'] == ['xgb_pieceweight']:  # if using a weighted tuning model
        ...
        model_stats = pd.DataFrame(
            columns=['Training_R2', 'Testing_R2', 'R2', 'Training_RMSE', 'Testing_RMSE', 'RMSE', 'Training_CRMSE',
                     'Testing_CRMSE', 'CRMSE', 'Training_MBE', 'Testing_MBE', 'MBE'])
        ...
        for p in settings['weighting_percentile']:
            for w in settings['weighting_weight']:
                ...
                percentile_model_stats[model_name] = pd.DataFrame(index=percentile_index,
                                                                  columns=['Training_R2', 'Testing_R2', 'R2',
                                                                           'Training_RMSE',
                                                                           'Testing_RMSE', 'RMSE', 'Training_CRMSE',
                                                                           'Testing_CRMSE',
                                                                           'CRMSE', 'Training_MBE', 'Testing_MBE', 'MBE'])

                if settings['models'] == ['rf_pieceweight']:
                    print(
                        f'Fitting colocation pod data to reference data using weighted random forest with p= {p_str} and w= {w_str}...')
                    percentile_model_stats, model_stats, y_train_predicted, y_test_predicted, y_predicted, current_model = weighting_grid.rf_pieceweight(
                        X_train, y_train,
                        X_test, y_test,
                        X_std, y, p,
                        w, model_name,
                    ...
                    print(
                        f'Fitting colocation pod data to reference data using weighted MLR with p= {p_str} and w= {w_str}...')
                    ...
                    print(
                        f'Fitting colocation pod data to reference data using weighted XGB with p= {p_str} and w= {w_str}...')
                ...
                y_predicted_TT.to_csv(os.path.join('Outputs', output_folder_name, f'{colo_pod_name}_{model_name}_colo_y_predicted.csv'))
                joblib.dump(current_model, os.path.join('Outputs', output_folder_name, f'{colo_pod_name}_{model_name}_model.joblib'))
                ...
                plotting_func.colo_plots_series(settings['colo_plot_list'], y_train, y_train_predicted, y_test,
                                                ...
                                                output_folder_name, settings['colo_run_name']+'_'+colo_pod_name, settings['unit'],
                                                current_model, list(X_std.columns), X_train, X_test)

    else:
        # if not using weighted models, proceed with this code:
        # first, establish a dataframe to save model statistics in
        model_stats = pd.DataFrame(index=settings['models'],
                                   columns=['Training_R2', 'Testing_R2', 'R2', 'Training_RMSE', 'Testing_RMSE', 'RMSE',
                                            'Training_CRMSE', 'Testing_CRMSE', 'CRMSE', 'Training_MBE', 'Testing_MBE',
                                            'MBE'])
        ...
        for model in settings['models']:
            percentile_model_stats[model] = pd.DataFrame(index=percentile_index,
                                                         columns=['Training_R2', 'Testing_R2', 'R2', 'Training_RMSE',
                                                                  'Testing_RMSE', 'RMSE',
                                                                  'Training_CRMSE', 'Testing_CRMSE', 'CRMSE',
                                                                  'Training_MBE', 'Testing_MBE', 'MBE'])
        ...
        for i, model_name in enumerate(settings['models']):
            ...
            percentile_model_stats, model_stats, y_train_predicted, y_test_predicted, y_predicted, current_model = model_func(
                X_train, y_train, X_test, y_test, X_std_values, y, model_name, model_stats, percentile_model_stats,
                settings)
            ...
            plotting_func.colo_plots_series(settings['colo_plot_list'], y_train, y_train_predicted, y_test,
                                            y_test_predicted, settings['pollutant'], model_name,
                                            output_folder_name, settings['colo_run_name']+'_'+colo_pod_name, settings['unit'], current_model,
                                            list(X_std.columns), X_train, X_test)

    # save out the model for later analysis and use in field data
    model_stats.to_csv(os.path.join('Outputs', output_folder_name, f'{colo_pod_name}_colo_model_stats_{settings["colo_run_name"]}.csv'),
                       index=True)

    with pd.ExcelWriter(
            os.path.join('Outputs', output_folder_name, f'{colo_pod_name}_colo_model_percentile_stats_{settings["colo_run_name"]}.xlsx'),
    ...
    if "colo_stats_plot" in settings['colo_plot_list']:
        if settings['models'] == ['rf_pieceweight'] or settings['models'] == ['lin_reg_pieceweight'] or settings['models'] == ['xgb_pieceweight']:
            # reset the models list so it includes each combo of p and w
            plotting_func.colo_stats_plot(list(model_stats.index), model_stats, settings['pollutant'], output_folder_name, settings['colo_run_name'] + '_' + colo_pod_name)
        else:
            plotting_func.colo_stats_plot(settings['models'], model_stats, settings['pollutant'], output_folder_name, settings['colo_run_name']+'_'+colo_pod_name)
