...
def load_deployment_log(colo_type):
    # Load deployment from either CSV or Excel file
    try:
        deployment_log = pd.read_csv(f"deployment_log_{colo_type}.csv")
    except FileNotFoundError:
        try:
            deployment_log = pd.read_excel(f"deployment_log_{colo_type}.xlsx")
        except FileNotFoundError:
    ...
    for i, file in enumerate(data_file_list):
        ...
        if os.path.exists(os.path.join(data_path, f'{file}.txt')):
            ...
            if 'date' in temp and 'time' in temp:
                ...
                temp['datetime'] = temp['date'].dt.date.astype('str') + ' ' + temp['time'].dt.time.astype('str')
